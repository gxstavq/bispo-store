...
test("somente administrador confirma manualmente após conferência explícita", () => {
  ...
  assert.match(service, /createSupabaseServiceClient/);
  assert.match(service, /confirm_pagbank_payment/);
  assert.match(service, /manual_payment_confirmed_by_admin/);
  assert.match(service, /changed_by: input\.actorUserId/);
...
test("confirmação e cancelamento preservam idempotência e reservas", () => {
  const service = source("services/orders/manual-payment.ts");
  const route = source("app/api/orders/[id]/route.ts");
  assert.match(service, /stock_deducted_at/);
  assert.match(service, /insertError\?\.code !== "23505"/);
  assert.match(service, /provider_event_id: `manual-payment-confirmed-\$\{order\.id\}`/);
  assert.match(service, /releaseOrderStockReservation\(orderId, "admin_cancelled_whatsapp_order"\)/);
